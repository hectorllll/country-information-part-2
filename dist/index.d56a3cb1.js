...
async function getDataCountries(country) {
    ...
    try {
        ...
        const result = await _axiosDefault.default.get('https://restcountries.com/v2/name/' + country);
...
function getCurrencies(currency) {
    if (currency.length === 2) return `and you can pay with ${currency[0].name} and ${currency[1].name}`;
    else return `and you can pay with ${currency[0].name}`;
}
function getLanguages(language) {
    if (language.length === 1) return `They speak ${language[0].name}`;
    else {
        let messageLang = 'They speak ';
        for(let i = 0; i < language.length; i++){
            if (i === 0) messageLang += language[i].name;
            else if (i > 0 && i < language.length - 1) messageLang += ', ' + language[i].name;
            else messageLang += ' and ' + language[i].name;
        }
        return messageLang;
    }
...
},{"./../utils":"60BxC"}],"ciiiV":[function(require,module,exports) {
...
},{}]},["1Mq12","5HwUs"], "5HwUs", "parcelRequire5e27")
